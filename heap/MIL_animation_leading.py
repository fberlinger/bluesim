...
## red leader for random initialization
#colors[:, 0, :] = cm.Reds(0.5)

## red leader for existing circle
#colors[int(0.15*data.shape[0]):, 0, :] = cm.Reds(0.5)

## red external leader
#colors[:int(0.15*data.shape[0]), 0, :] = np.zeros((int(0.15*data.shape[0]),4))
#colors[int(0.15*data.shape[0]):, 0, :] = cm.Reds(0.5)
