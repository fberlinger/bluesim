...
def log_series(series):
    """Logs the meta data of the experiment
    """
    np.savetxt('./logfiles/{}_{}_series.txt'.format(len(alphas), test_runs), series, fmt='%.2f', delimiter=',')
...
for alpha_iter, alpha in enumerate(alphas):
    for test_run in range(test_runs):
        ...
        while not success:
            ...
            for fish_id in range(no_fishes):
                ...
                fishes.append(Fish(fish_id, dynamics, environment, alpha))
                H.insert(fish_id, clock)

            # Simulate
            print('Simulation {} out of {}: alpha = {}, test_run = {}.'.format(alpha_iter*test_runs+test_run+1, len(alphas)*test_runs, alpha, test_run+1))
            ...
            # Save Data
            filename = str(alpha) + '_' + str(test_run)
            environment.log_to_file(filename)
            x_c, y_c, r_c, error, max_r, min_r, t_convergence = calc_circle(environment.tracking[:, :no_fishes*4], alpha, test_run)
            
            if t_convergence != -1:
                success = True
            else:
                failed_runs += 1
                filename = str(alpha) + '_' + str(test_run) + '_failed'
                environment.log_to_file(filename)
                log_meta()

        if alpha_iter*test_runs+test_run+1 == 1:
